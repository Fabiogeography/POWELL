Hmsc
function (Y, XFormula = ~., XData = NULL, X = NULL, XScale = TRUE, 
          XSelect = NULL, XRRRData = NULL, XRRRFormula = ~. - 1, XRRR = NULL, 
          ncRRR = 2, XRRRScale = TRUE, YScale = FALSE, studyDesign = NULL, 
          ranLevels = NULL, ranLevelsUsed = names(ranLevels), TrFormula = NULL, 
          TrData = NULL, Tr = NULL, TrScale = TRUE, phyloTree = NULL, 
          C = NULL, distr = "normal", truncateNumberOfFactors = TRUE) 
{
  hM = structure(list(Y = NULL, XData = NULL, XFormula = NULL, 
                      X = NULL, XScaled = NULL, XSelect = NULL, XRRRData = NULL, 
                      XRRRFormula = NULL, XRRR = NULL, XRRRScaled = NULL, YScaled = NULL, 
                      XInterceptInd = NULL, studyDesign = NULL, ranLevels = NULL, 
                      ranLevelsUsed = NULL, dfPi = NULL, rL = NULL, Pi = NULL, 
                      TrData = NULL, TrFormula = NULL, Tr = NULL, TrScaled = NULL, 
                      TrInterceptInd = NULL, C = NULL, phyloTree = NULL, distr = NULL, 
                      ny = NULL, ns = NULL, nc = NULL, ncNRRR = NULL, ncRRR = NULL, 
                      ncORRR = NULL, ncsel = NULL, nr = NULL, nt = NULL, nf = NULL, 
                      ncr = NULL, ncs = NULL, np = NULL, spNames = NULL, covNames = NULL, 
  ...
  if (!is.null(XData)) {
    ...
    if (inherits(XData, "list")) {
      ...
      if (!is.null(baddie <- BADvars(XData))) 
        stop("XData variables had bad types: ", 
             baddie)
  ...
  switch(class(hM$X)[1L], matrix = {
    if (is.null(colnames(hM$X))) {
      colnames(hM$X) = sprintf(sprintf("cov%%.%dd", 
  ...
  if (!XScale) {
    ...
    switch(class(hM$X)[1L], matrix = {
      ...
      hM$XScaled = lapply(split(XScaled, rep(1:hM$ns, each = hM$ny)), 
                          function(a) matrix(a, hM$ny, hM$nc))
    })
  }
  ...
  if (hM$ncRRR > 0) {
    ...
    for (k in seq_len(hM$ncRRR)) {
      hM$covNames = c(hM$covNames, paste0("XRRR_", 
                                          as.character(k)))
    }
  ...
  if (!is.null(phyloTree)) {
    corM = vcv.phylo(phyloTree, model = "Brownian", 
                     corr = TRUE)
  ...
  knownDistributions <- c("normal", "probit", "poisson", 
                          "lognormal poisson")
  if (length(distr) == 1) {
  ...
  if (length(distr) > 1 && !is.matrix(distr)) {
    ...
    for (i in 1:hM$ns) {
      switch(match.arg(distr[i], knownDistributions), normal = {
        distr2[i, 1] = 1
        distr2[i, 2] = 1
      }, probit = {
        distr2[i, 1] = 2
