...
##### AT RESOLUTION OF 10S TO 100S OF METRES AND BETWEEN ECOREGIONS ###########
...
vario1 <- variogram(RelCov_I ~ 1, data = dat, cutoff=1000) ## Lengthy unless data restricted or cutoff is implemented. For quicker calculations use dat[1:1000,]
...
vario2 <- variogram(TotalPctCover_I ~ 1, data = dat) 
plot(vario2$dist, vario2$gamma, xlab="Distance, m", ylab="gamma", main="TotalPctCover_I") 

vario3 <- variogram(Richness_I ~ 1, data = dat, cutoff=1000) 
plot(vario3$dist, vario3$gamma, xlab="Distance, m", ylab="gamma", main="Richness_I") 

vario4 <- variogram(DiversityShannon_I ~ 1, data = dat[!is.na(dat$DiversityShannon_I),], cutoff=1000) ## Continues to increase with distance
...
set.seed(0)
dat.trim <- dat[sample(nrow(dat), 10000),] ## Otherwise very lengthy
# 
# ### RelCov_I
...
cgm.sp <- spline.correlog(dat.trim$Long, dat.trim$Lat,  dat.trim$RelCov_I, resamp=5, quiet=FALSE, latlon=F, xmax=1000)
...
## If observations are univariate the spline (cross-)correlogram represents the generalization of the spatial (cross-)correlogram.
## If observations are multivariate the spline (cross-)correlogram represents the generalization of the Mantel (cross-)correlogram.
cgm.sp <- spline.correlog(dat.trim$Long, dat.trim$Lat,  dat.trim$TotalPctCover_I, resamp=5, quiet=FALSE, latlon=F, xmax=1000)
...
## If observations are univariate the spline (cross-)correlogram represents the generalization of the spatial (cross-)correlogram.
## If observations are multivariate the spline (cross-)correlogram represents the generalization of the Mantel (cross-)correlogram.
cgm.sp <- spline.correlog(dat.trim$Long, dat.trim$Lat,  dat.trim$Richness_I, resamp=5, quiet=FALSE, latlon=F, xmax=1000)
...
## If observations are univariate the spline (cross-)correlogram represents the generalization of the spatial (cross-)correlogram.
## If observations are multivariate the spline (cross-)correlogram represents the generalization of the Mantel (cross-)correlogram.
cgm.sp <- spline.correlog(dat.trim$Long, dat.trim$Lat,  dat.trim$DiversityShannon_I, resamp=5, quiet=FALSE, latlon=F, xmax=1000)
