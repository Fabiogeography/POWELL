...
# make spatial
div_sub$pos <- numFactor(scale(div_sub$Long), scale(div_sub$Lat))
# then create a dummy group factor to be used as a random term
div_sub$ID <- factor(rep(1, nrow(div_sub)))
# ^^^ could do something else to make a real group factor??
sp_mod <- glmmTMB(Richness_N_trans ~ RelCov_I_trans*npp_zz + 
                    RelCov_I_trans*hm_z + 
                    RelCov_I_trans*Richness_I_zz + Dataset +
                    mat(pos + 0 | ID), div_sub) # take some time to fit
# started at 12:09pm Friday
# crashed at 6:29pm :(

## Trying the spatial autocovariate approach
# adds a covariate that's the mean of neighboring values (same as lag model?)
# again - neighboring distance is somewhat arbirary and we can test several things out
# or - we can create sites for FIA and BLM data, and then add site as a RE???

# first, fit model
mod1 <- glmmTMB(Richness_N_trans ~ RelCov_I_trans*npp_zz + 
                  RelCov_I_trans*hm_z + 
                  RelCov_I_trans*Richness_I_zz + Dataset + (1|Group), div_sub)
summary(mod1)
# residuals
div_sub$resid <- residuals(mod1)
hist(div_sub$resid)
# make df spatial + project to units of distance
coordinates(div_sub) <- ~ Long + Lat
proj4string(div_sub) <- "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"
plot(div_sub)
div_sub_pj <- spTransform(div_sub, CRS = CRS("+proj=aea +lat_1=29.83333333333334 +lat_2=45.83333333333334 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"))
plot(div_sub_pj)
coordinates(div_sub_pj)
# plot variogram for residuals
library(gstat)
vario <- variogram(resid ~ 1, data = div_sub)
