> convertToCodaObject
function (hM, start = 1, spNamesNumbers = c(TRUE, TRUE), covNamesNumbers = c(TRUE, 
                                                                             TRUE), trNamesNumbers = c(TRUE, TRUE), Beta = TRUE, Gamma = TRUE, 
          V = TRUE, Sigma = TRUE, Rho = TRUE, Eta = TRUE, Lambda = TRUE, 
          Alpha = TRUE, Omega = TRUE, Psi = TRUE, Delta = TRUE) 
{
  ...
  for (i in 1:nt) {
    ...
    if (trNamesNumbers[2]) {
      trNames[i] = paste(trNames[i], sprintf("(T%d)", 
                                             i), sep = sep)
    }
  }
  ...
  postPsi = list()
  postDelta = list()
  getOmega = function(a, r = 1) return(crossprod(a$Lambda[[r]]))
  getEta = function(a, r = 1) return(a$Eta[[r]])
  getLambda = function(a, r = 1) return(a$Lambda[[r]])
  getAlpha = function(a, r = 1) return(a$Alpha[[r]])
  getPsi = function(a, r = 1) return(a$Psi[[r]])
  getDelta = function(a, r = 1) return(a$Delta[[r]])
  nfMat = matrix(0, nChains, nr)
  for (chain in 1:nChains) {
    postList = postListAll[[chain]][start:length(postListAll[[chain]])]
    for (r in seq_len(nr)) {
      nfMat[chain, r] = max(unlist(lapply(lapply(postList, 
                                                 getEta, r = r), ncol)))
    }
  }
  ...
  for (chain in 1:nChains) {
    ...
    if (Beta) {
      ...
      colnames(tmp) = sprintf("B[%s, %s]", covNames[rep(1:nc, 
                                                        ns)], spNames[rep(1:ns, each = nc)])
      postBeta[[chain]] = mcmc(tmp, thin = thin, start = start1, 
                               end = end1)
    }
    if (Gamma) {
      tmp = do.call(rbind, lapply(postList, function(a) as.vector(a$Gamma)))
      colnames(tmp) = sprintf("G[%s, %s]", covNames[rep(1:nc, 
                                                        nt)], trNames[rep(1:nt, each = nc)])
      postGamma[[chain]] = mcmc(tmp, thin = thin, start = start1, 
                                end = end1)
    }
    if (V) {
      tmp = do.call(rbind, lapply(postList, function(a) as.vector(a$V)))
      colnames(tmp) = sprintf("V[%s, %s]", covNames[rep(1:nc, 
                                                        nc)], covNames[rep(1:nc, each = nc)])
      postV[[chain]] = mcmc(tmp, thin = thin, start = start1, 
                            end = end1)
    }
    if (Sigma) {
      tmp = do.call(rbind, lapply(postList, function(a) a$sigma))
      colnames(tmp) = sprintf("Sig[%s]", spNames)
      postSigma[[chain]] = mcmc(tmp, thin = thin, start = start1, 
                                end = end1)
    }
    if (Rho) {
      postRho[[chain]] = mcmc(unlist(lapply(postList, function(a) a$rho)), 
                              thin = thin, start = start1, end = end1)
    }
    ...
    for (r in seq_len(nr)) {
      postNf = unlist(lapply(lapply(postList, getEta, r = r), 
                             ncol))
      ...
      if (Eta) {
        ...
        tmp2 = lapply(tmp1, function(A) cbind(A, matrix(0, 
                                                        nrow(A), nfMax[r] - ncol(A))))
        tmp = do.call(rbind, lapply(tmp2, as.vector))
        colnames(tmp) = sprintf("Eta%d[%s, factor%s]", 
                                r, levels(hM$dfPi[, r])[(rep(1:hM$np[r], nfMax[r]))], 
                                as.character(rep(1:nfMax[r], each = hM$np[r])))
        postEta1[[r]] = mcmc(tmp, thin = thin, start = start1, 
                             end = end1)
      }
      if (Lambda) {
        tmp1 = lapply(postList, getLambda, r = r)
        tmp2 = lapply(tmp1, function(A) rbind(A, matrix(0, 
                                                        nfMax[r] - nrow(A), ncol(A))))
        tmp = do.call(rbind, lapply(tmp2, function(A) as.vector(t(A)))) ## causes warning
        colnames(tmp) = sprintf("Lambda%d[%s, factor%s]", ## this causes the error
                                r, spNames[rep(1:ns, nfMax[r])], as.character(rep(1:nfMax[r], 
                                                                                  each = ns)))
        postLambda1[[r]] = mcmc(tmp, thin = thin, start = start1, 
                                end = end1)
      }
      if (Omega) {
        tmp = do.call(rbind, lapply(lapply(postList, 
                                           getOmega, r = r), as.vector))
        colnames(tmp) = sprintf("Omega%d[%s, %s]", 
                                r, spNames[rep(1:ns, ns)], spNames[(rep(1:ns, 
                                                                        each = ns))])
        postOmega1[[r]] = mcmc(tmp, thin = thin, start = start1, 
                               end = end1)
      }
      if (Alpha) {
        tmp1 = lapply(lapply(postList, getAlpha, r = r), 
                      function(a) hM$rL[[r]]$alphapw[a, 1])
        tmp2 = lapply(tmp1, function(a) c(a, rep(0, nfMax[r] - 
                                                   length(a))))
        tmp = do.call(rbind, tmp2)
        colnames(tmp) = sprintf("Alpha%d[factor%s]", 
                                r, as.character(1:nfMax[r]))
        postAlpha1[[r]] = mcmc(tmp, thin = thin, start = start1, 
                               end = end1)
      }
      if (Psi) {
        ...
        colnames(tmp) = sprintf("Psi%d[%s, factor%s]", 
                                r, spNames[rep(1:ns, nfMax[r])], as.character(rep(1:nfMax[r], 
                                                                                  each = ns)))
        postPsi1[[r]] = mcmc(tmp, thin = thin, start = start1, 
                             end = end1)
      }
      if (Delta) {
        tmp1 = lapply(lapply(postList, getDelta, r = r), 
                      function(a) c(a, rep(0, nfMax[r] - length(a))))
        tmp = do.call(rbind, tmp1)
        colnames(tmp) = sprintf("Delta%d[factor%s]", 
                                r, as.character(1:nfMax[r]))
        postDelta1[[r]] = mcmc(tmp, thin = thin, start = start1, 
                               end = end1)
      }
    }
